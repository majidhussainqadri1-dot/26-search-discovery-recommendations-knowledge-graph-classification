...
foreach ( array(
	...
	'sabri_file26_can_appeal_doctor_ranking',
) as $sabri_file26_boolean_authorization_filter ) {
	add_filter( $sabri_file26_boolean_authorization_filter, static function ( $value ) {
...
add_filter( 'rest_post_dispatch', static function ( $response, $server, $request ) {
	...
	if ( 0 === strpos( $route, '/sabri-search/v1/' ) && $response instanceof \WP_REST_Response ) {
		...
		if ( method_exists( $response, 'remove_header' ) ) { $response->remove_header( 'ETag' ); }
	}
	return $response;
}, PHP_INT_MAX, 3 );
...
register_activation_hook( __FILE__, static function () {
	$fail_activation = static function ( $message ) {
		...
		if ( function_exists( 'deactivate_plugins' ) ) { deactivate_plugins( plugin_basename( SABRI_FILE26_FILE ), true ); }
		wp_die( esc_html( $message ) );
	};
	add_filter( 'cron_schedules', static function ( $schedules ) { $schedules['sabri_file26_monthly'] = array( 'interval' => 30 * DAY_IN_SECONDS, 'display' => 'Every 30 days — File 26' ); return $schedules; } );
