...
| 19 | DEFECT | Search/Discover/Topic shortcode templates use fixed DOM IDs, so multiple instances on one document can duplicate IDs and break `aria-labelledby`/label ownership. Front-end status announcements target the first document-wide live region instead of the initiating component. Autocomplete blur does not invalidate/abort an in-flight request, allowing a late response to reopen the list after focus leaves the combobox. | Pending correction after this frozen ledger. |
...
- Round 19 review: **FROZEN — correction pending**

Round 20 must not begin until Round 19 correction, regression and exact-head CI are green.
