...
| 7 | DEFECT | `blocked/restricted` safety classes could still surface and first-page concentration limits could be violated by deferred overflow. | Ranking now excludes blocked/restricted classes and enforces a protected first-page concentration boundary. Regression: `tests/review-round-07-ranking-safety-diversity-second-cycle.php`. Initial closure CI exposed a missing brace introduced in the correction; fixed in `6fb924bbb7ea58638f0f9640a9681648ace6eac0`. Final exact-head CI GREEN `8186bdcba8e93ffcc49223c2ce65520381ec8e0c`. |
| 8 | DEFECT | Standalone term creation could leave a term row after alias failure; alias `INSERT IGNORE` could silently preserve an alias on the wrong/source term during merge; merge classification/alias read failures could be treated as empty sets before destructive cleanup; and direct `classify(..., approved)` could bypass the independent domain-review gate for high-impact labels. | FROZEN — correction pending. |

## Round status

- Completed reviews/corrections: **7/20**
- Active frozen round: **8/20 — correction pending**
- Defect rounds so far: **2, 3, 4, 5, 6, 7, 8**
- Clean rounds so far: **1**

Round 9 must not begin until Round 8 is corrected, regressed and exact-head CI is green.
