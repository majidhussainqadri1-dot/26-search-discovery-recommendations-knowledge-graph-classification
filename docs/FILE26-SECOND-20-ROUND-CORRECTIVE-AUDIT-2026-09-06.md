...
| 8 | DEFECT | Standalone term creation could leave a term after alias failure; alias `INSERT IGNORE` could preserve wrong ownership during merge; merge read failures could be treated as empty before destructive cleanup; direct high-impact approval could bypass independent domain review. | Public term creation is atomic; split uses a transaction-safe internal creation helper; merge checks classification reads and re-parents aliases with collision detection; direct high-impact approval requires `sabri_file26_classification_domain_reviewer_approved`. Regression: `tests/review-round-08-taxonomy-integrity-second-cycle.php`. Initial closure CI showed all functional assertions passing but rejected one interpolation-prone regression literal; harness corrected in `e7b94442777d725151c621901d209b80af1b10b2`. Final exact-head CI pending on this ledger head. |
