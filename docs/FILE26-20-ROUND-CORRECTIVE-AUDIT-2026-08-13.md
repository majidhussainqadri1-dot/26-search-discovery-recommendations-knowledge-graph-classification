...
| 20 | DEFECT | Corrective QA itself contained interpolation-prone regression literals; CI used older official GitHub Action runtimes; release evidence/docs required fresh closure. Exact-head CI then additionally exposed a missing closing brace in `Search::facets_from_rows()` and two stale wording/SHA-bound QA assertions. | Regression literals made interpolation-safe; official checkout/setup-node/upload-artifact pinned to current immutable Node 24 releases; missing PHP brace corrected; rollback dual-approval regression changed to semantic security checks; action-pin regressions aligned with the approved immutable Node 24 SHAs; final ledger/QA evidence refreshed. Exact-head CI and deterministic package remain the closing evidence for this round. |
