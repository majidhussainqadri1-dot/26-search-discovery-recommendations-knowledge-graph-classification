...
- **R4-D4 — Critical — shadow cutover does not prove monotonic source versions against the currently active derivative before replacing it.** A stale but internally self-consistent snapshot can pass its own `snapshot_count`/`snapshot_checksum`, then the cutover deletes a newer active document and promotes an older candidate version. Snapshot integrity is not the same as snapshot freshness. Each staged candidate, including restrictive markers, must fail closed when its source `object_version` is older than the currently active derivative for the same canonical object; tombstone precedence must also remain enforced.
