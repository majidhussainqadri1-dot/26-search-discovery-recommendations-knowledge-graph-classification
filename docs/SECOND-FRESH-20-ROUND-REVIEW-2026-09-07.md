...
- **R1-D4 — High — residual whitespace-coupled partial-state assertion:** exact-head run `34085851001` on `6397288f6c24a0e37adb8c3b3536722cc906979e` passed PHP syntax, normalization/ranking, and the entire architecture/traceability suite, then failed only the corrective assertion “Bounded corpus scans disclose truthful partial state.” The implementation contains the required `health => scan_limit` partial-domain signal, but `tests/corrective-contract-tests.php` still compared that one search token with exact whitespace.
- **R1-D5 — High — Round-03 search-integrity regression is formatting-coupled:** after R1-D4 correction, exact-head run `34086105329` reached the sequential regression suite and failed four assertions in `tests/review-round-03-search-integrity.php`: sensitivity classification before cache, sensitive-query shared-cache exclusion, deterministic equal-freshness tie-break, and canonical-key stable tie-break. Current `class-file26-search.php` implements all four semantics in compact formatting (`$sensitive_query=...`, `&&!$sensitive_query`, `if($at===$bt)`, `strcmp($a['canonical_key'],$b['canonical_key'])`). The failure is therefore test-format brittleness, not absence of the safeguards.
