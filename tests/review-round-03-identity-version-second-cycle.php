...
$checks = array(
	...
	array( $source, '$connector=sanitize_key($connector); $domain=sanitize_key($domain); $object_id=sanitize_text_field((string)$object_id);', 'tombstone normalizes the same identity before hashing' ),
	array( $source, '$key=self::canonical_key($connector,$domain,$object_id);', 'tombstone hashes normalized identity' ),
);
$failures = 0;
