...
$read = static function ( $path ) use ( $root ) {
	...
	if ( false === $content ) { fwrite( STDERR, "Unable to read $path\n" ); exit( 1 ); }
	return $content;
};
$passed = 0; $failed = 0;
function corrective_assert( $condition, $label ) { global $passed, $failed; if ( $condition ) { $passed++; echo "PASS: $label\n"; } else { $failed++; echo "FAIL: $label\n"; } }
...
corrective_assert(
	false !== strpos( $connectors, 'private function effective_status' ) &&
	false !== strpos( $connectors, "return 'proposed';" ) &&
	false !== strpos( $connectors, "\$existing['owner_file'] === \$manifest['owner_file']" ) &&
	false !== strpos( $connectors, "\$existing['contract_version'] === \$manifest['contract_version']" ),
	'Every new or changed connector begins in the proposed lifecycle state.'
);
...
foreach ( array( 'sabri_search_operator', 'sabri_taxonomy_curator', 'sabri_ranking_approver', 'sabri_search_auditor' ) as $role ) { corrective_assert( false !== strpos( $roles, $role ), "Dedicated separation-of-duties role exists: $role" ); }
corrective_assert(
...
foreach ( array( 'hide_item', 'hide_author', 'hide_topic', 'undo', 'opt_out', 'set_interests' ) as $control ) { corrective_assert( false !== strpos( $recommendations, $control ), "Recommendation control is implemented: $control" ); }
...
foreach ( array( 'country', 'city', 'language', 'specialization', 'educator', 'researcher' ) as $context ) { corrective_assert( false !== strpos( $doctor, "'$context'" ), "Doctor contextual ranking is supported: $context" ); }
...
foreach ( array( 'file03', 'file05', 'file06', 'file07', 'file08', 'file10', 'file11', 'file12', 'file15', 'file18', 'file21' ) as $owner ) { corrective_assert( false !== strpos( $owner_contracts, "'$owner'" ), "Required owner connector contract is declared: $owner" ); }
