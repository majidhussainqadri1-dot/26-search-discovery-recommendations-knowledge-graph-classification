...
$checks = array(
	...
	'last_event_version=IF(owner_file=VALUES(owner_file)ANDcontract_version=VALUES(contract_version),last_event_version,0)' => 'same-contract reload preserves event checkpoint',
	"health_state=IF(owner_file=VALUES(owner_file)ANDcontract_version=VALUES(contract_version),health_state,'unknown')" => 'same-contract reload preserves health state',
	'last_health=IF(owner_file=VALUES(owner_file)ANDcontract_version=VALUES(contract_version),last_health,NULL)' => 'same-contract reload preserves last health time',
);
$failures = 0;
foreach ( $checks as $needle => $label ) {
	if ( false === strpos( $normalized, preg_replace( '/\s+/', '', $needle ) ) ) { fwrite( STDERR, "FAIL: $label\n" ); $failures++; }
}
if ( false !== strpos( $normalized, "manifest=VALUES(manifest),last_event_version=0,health_state='unknown',last_health=NULL" ) ) {
