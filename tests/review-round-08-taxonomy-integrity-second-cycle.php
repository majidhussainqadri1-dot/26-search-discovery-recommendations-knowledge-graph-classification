...
if ( false !== strpos( $source, 'INSERT IGNORE INTO ' . "' . DB::table( 'term_aliases' )" ) ) {
	fwrite( STDERR, "FAIL: taxonomy alias ownership must not rely on INSERT IGNORE\n" );
	$failures++;
}
$transfer = strpos( $source, 'private function transfer_aliases' );
$cleanup = strpos( $source, "Source classification cleanup failed." );
if ( false === $transfer || false === $cleanup || $transfer < $cleanup ) {
	// The helper definition may occur later; the merge call must occur after classification cleanup and before commit.
	...
	if ( false === $call || false === $commit || $call > $commit ) {
		fwrite( STDERR, "FAIL: alias transfer must be part of the merge transaction before commit\n" );
		$failures++;
	}
}
if ( $failures ) { exit( 1 ); }
