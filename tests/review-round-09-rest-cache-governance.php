...
$checks = array(
	'contains_sensitive_query($query)' => 'search/suggest HTTP cache checks sensitive queries',
	...
	"'private, no-store'" => 'private responses are explicitly no-store',
);
$failures = 0;
foreach ( $checks as $needle => $label ) {
	if ( false === strpos( $source, $needle ) ) { fwrite( STDERR, "FAIL: $label\n" ); $failures++; }
}
if ( substr_count( $source, 'contains_sensitive_query($query)' ) < 2 ) {
