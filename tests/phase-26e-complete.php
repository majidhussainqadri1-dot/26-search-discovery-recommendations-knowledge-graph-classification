...
$query = (new QueryUnderstandingPipeline())->understand('Jigar ki sozish علاج ۱۲۳');
...
$pii = (new QueryUnderstandingPipeline())->understand('patient@example.com');
...
$documentA = new SearchDocument('file21:post:1','file21','v1','ur','published','https://sabrihomeopathy.com/post/1',[
    'title'=>'جگر کی سوزش کا تعلیمی سبق','content_type'=>'lesson','creator_id'=>'doctor-1','topics'=>['جگر','علاج'],'authority_score'=>90,'quality_score'=>90,'popularity_score'=>10,'trending_score'=>20,
],$public,new DateTimeImmutable('-1 day'));
$documentB = new SearchDocument('file10:video:2','file10','v1','en','published','https://sabrihomeopathy.com/video/2',[
    'title'=>'Liver Inflammation Educational Video','content_type'=>'video','creator_id'=>'doctor-2','topics'=>['jigar','liver'],'authority_score'=>80,'quality_score'=>80,'popularity_score'=>20,'trending_score'=>30,
],$public,new DateTimeImmutable('-2 days'));
$documentC = new SearchDocument('file12:pdf:3','file12','v1','ur','published','https://sabrihomeopathy.com/pdf/3',[
    'title'=>'جگر کی کتاب','content_type'=>'pdf','creator_id'=>'doctor-1','topics'=>['جگر'],'authority_score'=>70,'quality_score'=>75,
],$public,new DateTimeImmutable('-5 days'));
...
complete_assert($ranked[0]->document()->canonicalKey() === 'file21:post:1', 'Exact Urdu title relevance and authority should rank first.');
complete_assert(in_array('authoritative-source',$ranked[0]->explanations(),true), 'Ranking must expose an authoritative-source explanation.');

$recommendations = (new RecommendationEngine())->recommend([$documentA,$documentB,$documentC], new RecommendationContext(true,false,false,['جگر'],['doctor-1'],[],[],[],[],[],3));
...
$feedback = new FeedbackEvent(hash('sha256','fb-1'),hash('sha256','actor-1'),'file21:post:1','hide_item',hash('sha256','ctx'),new DateTimeImmutable('now'));
...
$root = new TaxonomyTerm('topic.health',1,['en'=>'Health','ur'=>'صحت'],['en'=>['wellness'],'ur'=>['تندرستی']],[],[],'Controlled health root.','file26-curated','active');
$taxonomy->register($root);
$liver = new TaxonomyTerm('topic.liver',1,['en'=>'Liver','ur'=>'جگر'],['en'=>['hepatic'],'ur'=>[]],['topic.health'],[],'Controlled liver topic.','file26-curated','active');
...
} catch (Sabri\File26\Support\InvariantViolation) { ++$assertions; }
...
$graph->putNode($documentA); $graph->putNode($documentB);
...
complete_assert(count($health) === 9, 'Default connector registry must include all implemented public owner adapters.');
complete_assert(isset($health['file09.doctors'],$health['file21.publications'],$health['file18.marketplace']), 'Doctor, publication and marketplace connectors must be registered.');
foreach($health as $row) complete_assert($row['version']==='1.0.0' && $row['schema_version']==='1.0.0', 'Connector contracts must be frozen at 1.0.0.');
...
try { ChangeEvent::create('event-key-2','file21','file21:post:1','v3','deleted',new DateTimeImmutable('now'),3,['title'=>'leak']); throw new RuntimeException('Deletion payload should fail.'); } catch (Sabri\File26\Support\InvariantViolation) { ++$assertions; }

complete_assert(SABRI_FILE26_VERSION === '1.0.0' && SABRI_FILE26_SCHEMA_VERSION === '1.0.0', 'Runtime and schema identity must be 1.0.0.');
complete_assert(SABRI_FILE26_STAGE === 'complete-runtime', 'Runtime stage must truthfully identify complete code scope.');
