...
printf '[7/14] Sequential corrective review regressions\n'
shopt -s nullglob
...
printf '[10/14] Required release evidence and source hygiene\n'
for file in README.md readme.txt CHANGELOG.md DECISION-LOG.md LICENSE docs/ARCHITECTURE.md docs/CONNECTOR-CONTRACT.md docs/REST-CONTRACT.md docs/SECURITY-THREAT-MODEL.md docs/PRIVACY-RETENTION.md docs/MIGRATION.md docs/ROLLBACK.md docs/STAGING-ACCEPTANCE.md docs/REQUIREMENTS-TRACEABILITY.md docs/REVIEW-AND-CORRECTION-1.0.0.md docs/REVIEW-AND-CORRECTION-1.1.0.md docs/NEW-GOVERNING-PLANS-COMPLETION-1.2.0.md docs/REVIEW-AND-CORRECTION-1.2.0-ROUND-1.md docs/REVIEW-AND-CORRECTION-1.2.0-ROUND-2.md docs/FILE26-20-ROUND-CORRECTIVE-AUDIT-2026-08-13.md docs/FILE26-20-ROUND-CORRECTIVE-AUDIT-2026-09-06.md docs/QA-REPORT.md docs/SBOM.md; do test -s "$ROOT/$file"; done
if test -e "$ROOT/MANIFEST.sha256"; then echo 'FAIL: source MANIFEST.sha256 must be generated from the exact package snapshot, not committed/stale'; exit 1; fi
...
printf '[12/14] Deterministic double build without source mutation\n'
...
printf '[14/14] Clean-extract QA and exact source/package manifest parity\n'
...
(cd "$ROOT" && sha256sum -c "$PACKAGE/MANIFEST.sha256" >/dev/null)

rm -rf "$ROOT/release"
mkdir -p "$ROOT/release"
cp "$TMP/a.zip" "$ROOT/release/26-sabri-file26-search-discovery-1.2.0.zip"
(cd "$ROOT/release" && sha256sum 26-sabri-file26-search-discovery-1.2.0.zip > CHECKSUMS.sha256)
