...
trait Future_Multimodal_Trait {
	public function multimodal_search( \WP_REST_Request $request ) {
		...
		$reference = array( 'owner' => isset( $asset['owner'] ) ? sanitize_key( (string) $asset['owner'] ) : '', 'object_id' => isset( $asset['object_id'] ) ? sanitize_text_field( (string) $asset['object_id'] ) : '', 'object_version' => isset( $asset['object_version'] ) ? max( 0, (int) $asset['object_version'] ) : 0, 'kind' => $asset_kind );
	...
	public function segment_search( \WP_REST_Request $request ) {
		...
		foreach ( array_slice( $provided['segments'], 0, 50 ) as $segment ) {
			...
			$valid[] = array( 'owner' => sanitize_key( (string) $segment['owner'] ), 'object_id' => sanitize_text_field( (string) $segment['object_id'] ), 'canonical_url' => $canonical_url, 'title' => $title, 'excerpt' => $excerpt, 'position' => $position, 'provenance' => substr( sanitize_text_field( (string) $segment['provenance'] ), 0, 300 ) );
		}
