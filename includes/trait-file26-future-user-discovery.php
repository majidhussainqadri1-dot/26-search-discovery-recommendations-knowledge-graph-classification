...
/** F26-FUT-18 — transparent recommendation controls and sample explanations. */
	public function recommendation_transparency( \WP_REST_Request $request ) {
		...
		$controls = get_user_meta( $user_id, self::META_DISCOVERY, true );
		$controls = is_array( $controls ) ? $controls : array( 'breadth' => 'standard', 'less_personalization' => false );
		if ( 'POST' === strtoupper( $request->get_method() ) ) {
			...
			update_user_meta( $user_id, self::META_DISCOVERY, $controls );
		}
...
/** F26-FUT-19 — anti-filter-bubble breadth modes with deterministic source/author diversification. */
	public function discovery_breadth( \WP_REST_Request $request ) {
		...
		if ( 'POST' === strtoupper( $request->get_method() ) && isset( $params['breadth'] ) ) {
			$breadth = sanitize_key( (string) $params['breadth'] ); if ( in_array( $breadth, array( 'standard', 'diverse', 'broad' ), true ) ) { $controls['breadth'] = $breadth; update_user_meta( $user_id, self::META_DISCOVERY, $controls ); }
		}
...
/** F26-FUT-20 — geo/language/availability discovery; File 07/08 retain truth. */
	public function geo_availability( \WP_REST_Request $request ) {
		...
		foreach ( array( 'country', 'location', 'language', 'specialization' ) as $key ) {
			if ( ! empty( $params[ $key ] ) ) { $filters[ $key ] = sanitize_text_field( (string) $params[ $key ] ); }
		}
		$availability_request = array();
		foreach ( array( 'availability', 'timezone', 'mode' ) as $key ) {
			if ( ! empty( $params[ $key ] ) ) { $availability_request[ $key ] = sanitize_text_field( (string) $params[ $key ] ); }
		}
		...
		if ( $owner_available ) {
			$filters = array_merge( $filters, $this->sanitize_filters( $owner_constraints ) );
		}
		$result = $this->base_search( $this->query( $params ), array( 'filters' => $filters, 'limit' => 30 ) );
		if ( is_wp_error( $result ) ) { return $result; }
		return array(
			'state' => $availability_request && ! $owner_available ? 'owner_availability_provider_unavailable' : 'ok',
			'filters' => $filters,
			'availability_request' => $availability_request,
			'availability_provider_available' => $owner_available,
			'availability_claims_suppressed' => $availability_request && ! $owner_available,
			'results' => $this->safe_results( (array) $result['results'] ),
			'doctor_truth_owner' => 'File 07',
			'clinic_and_appointment_truth_owner' => 'File 08',
			'availability_truth_computed_by_file26' => false,
			'click_time_owner_revalidation_required' => true,
		);
	}
