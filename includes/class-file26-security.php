...
final class Security {
	...
	public function require_step_up( $purpose ) {
		...
		return (bool) apply_filters( 'sabri_file26_step_up_authorized', false, get_current_user_id(), (string) $purpose );
	}
	...
	public function safe_resource_url( $url, $purpose = 'resource' ) {
		...
		if ( $same_origin ) { return $same_origin; }
		$url = esc_url_raw( $url, array( 'http', 'https' ) );
		$parts = $url ? wp_parse_url( $url ) : false;
		if ( ! $parts || empty( $parts['scheme'] ) || empty( $parts['host'] ) || isset( $parts['user'] ) || isset( $parts['pass'] ) ) { return ''; }
	...
	public function sign_cursor( array $payload ) {
		$now = time();
		$ttl = max( 60, min( DAY_IN_SECONDS, (int) apply_filters( 'sabri_file26_cursor_ttl_seconds', 1800 ) ) );
		$payload['v'] = 1;
		$payload['iat'] = $now;
		$payload['exp'] = $now + $ttl;
	...
	public function verify_cursor( $cursor ) {
		if ( ! is_string( $cursor ) || false === strpos( $cursor, '.' ) ) { return false; }
		...
		if ( ! is_array( $payload ) || ! isset( $payload['v'], $payload['iat'], $payload['exp'] ) || 1 !== (int) $payload['v'] ) { return false; }
		$now = time();
		if ( (int) $payload['iat'] > $now + 60 || (int) $payload['exp'] < $now || (int) $payload['exp'] <= (int) $payload['iat'] ) { return false; }
		return $payload;
	}
