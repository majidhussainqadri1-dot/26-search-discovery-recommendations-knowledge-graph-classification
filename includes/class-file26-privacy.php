...
final class Privacy {
	...
	public function export( $email, $page = 1 ) {
		...
		$done = count( $feedback ) < $this->page_size && count( $appeals ) < $this->page_size;
	...
	public function erase( $email, $page = 1 ) {
		...
		try {
			...
			if ( false === $removed_feedback || false === $removed_profile ) {
				throw new \RuntimeException( 'Recommendation erasure failed.' );
			}
			$removed_any = (bool) ( $removed_feedback || $removed_profile );
			if ( $appeal_count ) {
				...
				if ( false === $updated || (int) $updated !== $appeal_count ) {
					throw new \RuntimeException( 'Ranking appeal redaction was incomplete.' );
				}
				$retained = true;
				$removed_any = true;
			}
			...
			return array(
				...
				'messages' => array( __( 'File 26 erasure could not be completed atomically; no partial success is reported.', 'sabri-file26' ) ),
				'done' => false,
			);
		}
