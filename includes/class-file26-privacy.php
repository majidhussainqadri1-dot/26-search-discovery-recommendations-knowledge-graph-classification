...
final class Privacy {
	...
	private $future_meta_keys = array(
		'sabri_file26_research_trails_v1' => 'Research trails',
		'sabri_file26_saved_search_alerts_v1' => 'Saved search alerts',
		'sabri_file26_search_history_sync_v1' => 'Opt-in server search history',
		'sabri_file26_search_history_sync_opt_in_v1' => 'Server search-history opt-in',
		'sabri_file26_discovery_controls_v1' => 'Discovery controls',
	);
	...
	public function export($email,$page=1){
		...
		if(1===$page){foreach($this->future_meta_keys as $meta_key=>$label){if(!metadata_exists('user',$user->ID,$meta_key)){continue;}$value=get_user_meta($user->ID,$meta_key,true);if(is_array($value)||is_object($value)){$value=wp_json_encode($value);}elseif(is_bool($value)){$value=$value?'yes':'no';}else{$value=(string)$value;}$data[]=array('group_id'=>'sabri-file26-future-account-data','group_label'=>__('Future Search Account Data','sabri-file26'),'item_id'=>'future-meta-'.hash('sha256',$meta_key.'|'.$user->ID),'data'=>array(array('name'=>__($label,'sabri-file26'),'value'=>$value)));}}
	...
	public function erase($email,$page=1){
		...
		try{
			...
			foreach($this->future_meta_keys as $meta_key=>$label){if(!metadata_exists('user',$user->ID,$meta_key)){continue;}delete_user_meta($user->ID,$meta_key);if(metadata_exists('user',$user->ID,$meta_key)){throw new \RuntimeException('Future account-owned metadata erasure failed.');}$removed_any=true;}
		...
		$messages=array(__('File 26 recommendation controls, feedback and account-owned Future Search data were erased.','sabri-file26'));if($retained){$messages[]=__('Ranking appeal text and identity were redacted; a minimal policy, status and fairness record was retained for audit integrity.','sabri-file26');}
