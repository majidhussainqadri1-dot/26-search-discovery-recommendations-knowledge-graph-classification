...
trait Future_Search_Core_Trait {
	public function conversational_grounded_search( \WP_REST_Request $request ) {
		...
		if ( ! $clinical && ! $this->sensitive_query( $q ) && $sources ) {
			...
			if ( is_array( $candidate ) && ! empty( $candidate['answer'] ) && isset( $candidate['safety_attestation'] ) && 'grounded_non_prescriptive' === $candidate['safety_attestation'] ) {
				...
				if ( $citations ) {
					...
					$answer = array( 'text' => $text, 'citations' => $citations, 'mode' => 'provider_grounded' ); $provider_state = 'grounded';
				}
			}
		}
		if ( null === $answer && $sources ) {
			...
			$answer = array( 'text' => '', 'citations' => array_values( array_filter( array_column( $extracts, 'key' ) ) ), 'extracts' => $extracts, 'mode' => 'extractive_only' ); $provider_state = 'extractive_only';
		}
	...
	public function semantic_rerank( \WP_REST_Request $request ) {
		...
		if ( is_array( $scores ) && $scores ) {
			...
			unset( $item );
			usort( $candidates, static function ( $a, $b ) { $as = null !== $a['semantic_rerank_score'] ? $a['semantic_rerank_score'] : ( isset( $a['score'] ) ? (float) $a['score'] : 0 ); $bs = null !== $b['semantic_rerank_score'] ? $b['semantic_rerank_score'] : ( isset( $b['score'] ) ? (float) $b['score'] : 0 ); return $as === $bs ? 0 : ( $as > $bs ? -1 : 1 ); } );
		}
