 ...
 * File 26 orchestrates manipulation-resistant rankings without changing identity truth.
 */
final class Doctor_Ranking {
	...
	public function recompute( $reason = 'scheduled_monthly' ) {
		...
		foreach ( $rows as $row ) {
			...
			if ( ! is_array( $payload ) || empty( $payload['verified_doctor'] ) ) {
				continue;
			}
			$scored[] = array(
		...
		foreach ( $scored as $item ) {
			...
			$wpdb->update(
				...
				array( 'canonical_key' => $item['key'] ),
				array( '%s', '%s' ),
				array( '%s' )
			);
		}
		DB::update_settings( array(
			'doctor_ranking_last_run' => DB::now(),
			'doctor_ranking_policy_version' => $policy['version'],
		) );
		$this->security->audit( 'doctor_ranking_recomputed', array(
	...
	/**
	 * Public ranking query. Contextual views are filters over the globally eligible
	 * doctor population and never replace or erase the global rank/tier.
	 */
	public function directory( array $request = array() ) {
		...
		if ( ! in_array( $context, $allowed_contexts, true ) || ! in_array( $tier, $allowed_tiers, true ) ) {
			return new \WP_Error( 'file26_invalid_doctor_ranking_view', 'Invalid doctor-ranking context or tier.', array( 'status' => 400 ) );
		}
		if ( in_array( $context, array( 'country', 'city', 'language', 'specialization' ), true ) && '' === $value ) {
			return new \WP_Error( 'file26_ranking_context_value_required', 'This contextual ranking requires a value.', array( 'status' => 400 ) );
		}
		...
		if ( ! empty( $request['cursor'] ) ) {
			...
			if ( ! $cursor || empty( $cursor['h'] ) || empty( $cursor['p'] ) || $cursor['p'] !== $policy['version'] || ! hash_equals( $cursor_context, (string) $cursor['h'] ) ) {
				return new \WP_Error( 'file26_invalid_cursor', 'The doctor-ranking cursor is invalid or expired.', array( 'status' => 400 ) );
			}
		...
		foreach ( $this->eligible_rows() as $row ) {
			...
			if ( ! $this->matches_tier( $global_rank, $tier ) ) {
				continue;
			}
			$scored[] = array(
				'key' => $row['canonical_key'],
		...
		foreach ( $scored as &$item ) {
			...
			$item['global_tier'] = $this->tier_for_rank( $item['global_rank'] );
			$item['explanation'] = $this->explain( $item['payload'], $policy );
			unset( $item['payload'] );
		}
		unset( $item );
		$total = count( $scored );
		...
		return array(
			...
			'results' => $page,
			'next_cursor' => $has_more ? $this->security->sign_cursor( array(
				'o' => $offset + $limit,
				'p' => $policy['version'],
				'h' => $cursor_context,
			) ) : null,
			'global_tiers_preserved' => true,
		);
	}

	public function score( array $payload, array $weights = array() ) {
		$weights = $weights ? $weights : $this->policy()['weights'];
		$score = 0.0;
		foreach ( $weights as $field => $weight ) {
			$value = isset( $payload[ $field ] ) ? min( 1.0, max( 0.0, (float) $payload[ $field ] ) ) : 0.0;
			$score += $value * max( 0.0, (float) $weight );
		}
		// Donation, payment, promotion, followers and Founder preference are deliberately absent.
	...
	public function policy() {
		...
		$row = $wpdb->get_row(
			"SELECT version,features_json FROM " . DB::table( 'ranking_policies' ) . " WHERE context_name='doctor_global' AND audience='public' AND status='active' ORDER BY effective_at DESC,id DESC LIMIT 1",
			ARRAY_A
	...
	private function matches_tier( $rank, $tier ) {
		if ( $rank <= 0 ) {
			return 'all_verified' === $tier;
		}
		if ( 'top_10' === $tier ) {
			return $rank <= 10;
		}
	...
	private function tier_for_rank( $rank ) {
		if ( $rank > 0 && $rank <= 10 ) {
			return array( 'key' => 'top_10', 'label' => __( 'Top 10 Verified Doctors', 'sabri-file26' ) );
		}
		if ( $rank > 0 && $rank <= 100 ) {
			return array( 'key' => 'top_100', 'label' => __( 'Top 100 Verified Doctors', 'sabri-file26' ) );
		}
		if ( $rank > 0 && $rank <= 1000 ) {
			return array( 'key' => 'top_1000', 'label' => __( 'Top 1000 Verified Doctors', 'sabri-file26' ) );
		}
		return array( 'key' => 'all_verified', 'label' => __( 'All Verified Doctors', 'sabri-file26' ) );
	}

	private function explain( array $payload, array $policy ) {
		$reasons = array();
		$labels = array(
			'qualification_score' => __( 'Verified qualifications', 'sabri-file26' ),
			'experience_score' => __( 'Professional experience', 'sabri-file26' ),
			'patient_verified_review_score' => __( 'Patient-verified reviews', 'sabri-file26' ),
			'ethical_conduct_score' => __( 'Ethical-conduct record', 'sabri-file26' ),
			'knowledge_contribution_score' => __( 'Knowledge contribution', 'sabri-file26' ),
			'responsiveness_score' => __( 'Responsiveness', 'sabri-file26' ),
			'profile_completeness_score' => __( 'Profile completeness', 'sabri-file26' ),
			'complaint_appeal_outcome_score' => __( 'Complaint and appeal outcomes', 'sabri-file26' ),
			'manipulation_resistant_engagement_score' => __( 'Manipulation-resistant engagement', 'sabri-file26' ),
		);
		$components = array();
		foreach ( $policy['weights'] as $field => $weight ) {
			$value = isset( $payload[ $field ] ) ? min( 1.0, max( 0.0, (float) $payload[ $field ] ) ) : 0.0;
			$components[ $field ] = $value * $weight;
		}
		arsort( $components );
		foreach ( array_slice( $components, 0, 4, true ) as $field => $component ) {
			if ( $component > 0 && isset( $labels[ $field ] ) ) {
				$reasons[] = $labels[ $field ];
			}
		}
		$reasons[] = __( 'Donation, payment, paid promotion, follower count and Founder favoritism are excluded', 'sabri-file26' );
		return $reasons;
	}

	private function sort_scored( array &$scored ) {
