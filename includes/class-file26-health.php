...
final class Health {
	...
	private function cron_event($hook){return function_exists('wp_get_scheduled_event')?wp_get_scheduled_event($hook):false;}
	public function snapshot() {
		...
		$unknown=in_array('missing',$tables,true);$connector_health=$this->connectors->health_snapshot();$degraded=false;foreach($connector_health as $connector){if('active'===$connector['status']&&!in_array($connector['state'],array('healthy','ok'),true)){$degraded=true;}}
		...
		$expected_cron=array('queue'=>array(DB::CRON_QUEUE,'hourly'),'reconcile'=>array(DB::CRON_RECONCILE,'twicedaily'),'retention'=>array(DB::CRON_RETENTION,'daily'),'doctor_ranking'=>array(DB::CRON_DOCTOR_RANKING,'sabri_file26_monthly'));
		$cron=array();$cron_recurrence_drift=false;foreach($expected_cron as $key=>$definition){$event=$this->cron_event($definition[0]);$cron[$key]=$event&&isset($event->timestamp)?(int)$event->timestamp:(wp_next_scheduled($definition[0])?:null);if($activated&&(!$event||!isset($event->schedule)||$definition[1]!==$event->schedule)){$cron_recurrence_drift=true;}}
		$cron_missing=$activated&&in_array(null,$cron,true);
		$physical_schema_ok=DB::schema_physical_ok();$appeal_schema_ok=Doctor_Appeals::schema_physical_ok();
		$schema_drift=!$physical_schema_ok||!$appeal_schema_ok||SABRI_FILE26_SCHEMA_VERSION!==(string)get_option(DB::OPTION_SCHEMA)||Doctor_Appeals::SCHEMA_VERSION!==(string)get_option(Doctor_Appeals::OPTION_SCHEMA);
		$role_model_ok=Roles::model_ok();
		$status=$unknown||$schema_drift?'unavailable':($db_read_failed||$degraded||$dead_letter||$stale_running||$cron_missing||$cron_recurrence_drift||($activated&&!$owner_contracts_ready)||($activated&&!$role_model_ok)?'degraded':($activated?'healthy':'inactive'));
		return array('status'=>$status,'plugin_version'=>SABRI_FILE26_VERSION,'schema_version'=>get_option(DB::OPTION_SCHEMA),'contract_version'=>SABRI_FILE26_CONTRACT_VERSION,'activated'=>$activated,'tables'=>$tables,'counts'=>$counts,'database_read_failed'=>$db_read_failed,'dead_letter_jobs'=>$dead_letter,'stale_running_jobs'=>$stale_running,'schema_drift'=>$schema_drift,'physical_schema_ok'=>$physical_schema_ok,'appeal_schema_ok'=>$appeal_schema_ok,'role_model_ok'=>$role_model_ok,'connectors'=>$connector_health,'owner_contracts'=>$owner_readiness,'owner_contracts_ready'=>$owner_contracts_ready,'cron'=>$cron,'cron_missing'=>$cron_missing,'cron_recurrence_drift'=>$cron_recurrence_drift,'doctor_ranking'=>array('policy_version'=>DB::setting('doctor_ranking_policy_version','doctor-global-1.0'),'last_run'=>DB::setting('doctor_ranking_last_run',''),'appeals_schema'=>get_option(Doctor_Appeals::OPTION_SCHEMA)),'claims'=>array('staging_accepted'=>false,'live_deployed'=>false,'operational'=>false),'timestamp_utc'=>DB::now());
	}
}
