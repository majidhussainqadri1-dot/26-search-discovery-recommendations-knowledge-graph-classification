...
final class Indexer {
	...
	public static function canonical_key( $connector, $domain, $object_id ) {
		return hash( 'sha256', sanitize_key( $connector ) . '|' . sanitize_key( $domain ) . '|' . sanitize_text_field( (string) $object_id ) );
	}

	private static function normalize_positive_version( $value ) {
		$version = filter_var( $value, FILTER_VALIDATE_INT, array( 'options' => array( 'min_range' => 1 ) ) );
		return false === $version ? false : (int) $version;
	}

	private static function normalize_nonnegative_sequence( $value ) {
		$sequence = filter_var( $value, FILTER_VALIDATE_INT, array( 'options' => array( 'min_range' => 0 ) ) );
		return false === $sequence ? false : (int) $sequence;
	}

	private function checked_scalar( $sql, $error_code, $message ) {
		global $wpdb;
		$wpdb->last_error = '';
	...
	private function checked_row( $sql, $error_code, $message ) {
		global $wpdb;
		$wpdb->last_error = '';
	...
	public function upsert( array $document ) {
		...
		try {
			$tombstone_raw = $this->checked_scalar(
				$wpdb->prepare( 'SELECT object_version FROM ' . DB::table( 'tombstones' ) . ' WHERE canonical_key=%s', $document['canonical_key'] ),
				'file26_index_precedence_read_failed',
				'Tombstone precedence could not be read safely.'
			);
			if ( is_wp_error( $tombstone_raw ) ) { return $tombstone_raw; }
			...
			$existing = $this->checked_row(
				$wpdb->prepare( "SELECT object_version,source_event_sequence,checksum FROM $table WHERE canonical_key=%s", $document['canonical_key'] ),
				'file26_index_existing_read_failed',
				'Existing search document state could not be read safely.'
			);
			if ( is_wp_error( $existing ) ) { return $existing; }
			if ( $existing ) {
				if ( (int) $document['object_version'] < (int) $existing['object_version'] ) {
					$this->security->audit( 'index_event_ignored', array( 'object_type' => 'document', 'object_key' => $document['canonical_key'], 'reason' => 'stale_object_version' ) );
					return true;
				}
			...
			if ( false === $wpdb->query( 'START TRANSACTION' ) ) { return new \WP_Error( 'file26_index_transaction_failed', 'Search document transaction could not start.' ); }
			try {
				if ( false === $wpdb->query( $sql ) ) { throw new \RuntimeException( 'Document write failed.' ); }
				if ( false === $wpdb->delete( DB::table( 'tombstones' ), array( 'canonical_key' => $document['canonical_key'] ), array( '%s' ) ) ) { throw new \RuntimeException( 'Stale tombstone removal failed.' ); }
				if ( false === $this->upsert_node( $document ) ) { throw new \RuntimeException( 'Node projection write failed.' ); }
				if ( false === $wpdb->query( 'COMMIT' ) ) { throw new \RuntimeException( 'Document transaction commit failed.' ); }
			} catch ( \Throwable $e ) {
				$wpdb->query( 'ROLLBACK' );
				return new \WP_Error( 'file26_index_write_failed', 'Search document and derivative projection could not be indexed atomically.' );
			}
	...
	private function sanitize_document( array $document ) {
		...
		$object_id = sanitize_text_field( (string) $document['object_id'] );
		if ( '' === $connector || '' === $domain || '' === $object_id || strlen( $domain ) > 64 || strlen( $object_id ) > 191 ) { return new \WP_Error( 'file26_invalid_document_identity', 'Connector, domain and object identity must remain valid after normalization.' ); }
		$version = self::normalize_positive_version( $document['object_version'] );
		if ( false === $version ) { return new \WP_Error( 'file26_invalid_object_version', 'Object version must be a positive integer.' ); }
		$sequence = isset( $document['source_event_sequence'] ) ? self::normalize_nonnegative_sequence( $document['source_event_sequence'] ) : $version;
		if ( false === $sequence ) { return new \WP_Error( 'file26_invalid_event_sequence', 'Source event sequence must be a non-negative integer.' ); }
		...
		$now = DB::now();
		$data = array(
			'canonical_key' => self::canonical_key( $connector, $domain, $object_id ), 'connector_slug' => $connector, 'domain_name' => $domain, 'object_id' => $object_id,
			'object_version' => $version, 'entity_type' => sanitize_key( $document['entity_type'] ), 'locale' => substr( sanitize_text_field( $document['locale'] ), 0, 20 ),
			'state' => $state, 'visibility' => $visibility, 'title' => $title, 'excerpt' => $excerpt, 'normalized_title' => $this->normalizer->normalize( $title ),
			'normalized_body' => $this->normalizer->normalize( $body ), 'canonical_url' => $url, 'author_key' => isset( $document['author_key'] ) ? sanitize_text_field( $document['author_key'] ) : '',
			...
			'country' => isset( $document['country'] ) ? sanitize_text_field( $document['country'] ) : '', 'location' => isset( $document['location'] ) ? sanitize_text_field( $document['location'] ) : '',
			'availability' => isset( $document['availability'] ) ? sanitize_key( $document['availability'] ) : '', 'quality_score' => isset( $document['quality_score'] ) ? min( 1, max( 0, (float) $document['quality_score'] ) ) : 0,
			...
			'freshness_at' => isset( $document['freshness_at'] ) ? gmdate( 'Y-m-d H:i:s', strtotime( $document['freshness_at'] ) ) : $now, 'safety_class' => isset( $document['safety_class'] ) ? sanitize_key( $document['safety_class'] ) : 'general',
			'payload' => wp_json_encode( $payload ), 'source_event_id' => isset( $document['source_event_id'] ) ? sanitize_text_field( $document['source_event_id'] ) : '', 'source_event_sequence' => $sequence,
			'indexed_at' => $now, 'updated_at' => $now,
		);
		$data['checksum'] = hash( 'sha256', wp_json_encode( $data ) );
		return $data;
	}
	...
	private function sanitize_payload( array $payload ) {
		...
		foreach ( $allowed as $key ) {
			...
			if ( in_array( $key, array( 'image_url', 'download_url' ), true ) ) { $clean[ $key ] = $this->security->safe_resource_url( $payload[ $key ], $key ); }
		...
		if ( empty( $clean['download_allowed'] ) ) { unset( $clean['download_url'] ); }
		return $clean;
	}

	public function restrict( $connector, $domain, $object_id, $object_version, $reason = 'restricted' ) {
		return $this->tombstone( $connector, $domain, $object_id, $object_version, $reason );
	}
	...
	public function tombstone( $connector, $domain, $object_id, $object_version, $reason = 'deleted' ) {
		...
		$version = self::normalize_positive_version( $object_version );
		if ( '' === $connector || '' === $domain || '' === $object_id || strlen( $domain ) > 64 || strlen( $object_id ) > 191 || false === $version ) {
			return new \WP_Error( 'file26_invalid_tombstone_identity', 'A valid normalized connector, domain, object identity and positive version are required for revocation.' );
		}
		...
		try {
			...
			$existing_raw = $this->checked_scalar(
				$wpdb->prepare( "SELECT object_version FROM $document_table WHERE canonical_key=%s", $key ),
				'file26_tombstone_precedence_read_failed',
				'Existing document version could not be read safely before revocation.'
			);
			if ( is_wp_error( $existing_raw ) ) { return $existing_raw; }
			$existing_version = null === $existing_raw ? 0 : (int) $existing_raw;
			if ( $existing_version > $version ) { return true; }
			$now = DB::now();
			$days = max( 90, min( 180, (int) DB::setting( 'tombstone_retention_days', 180 ) ) );
			$expires = gmdate( 'Y-m-d H:i:s', time() + ( $days * DAY_IN_SECONDS ) );
			if ( false === $wpdb->query( 'START TRANSACTION' ) ) { return new \WP_Error( 'file26_tombstone_failed', 'Document revocation transaction could not start.' ); }
			try {
				$sql = $wpdb->prepare(
					"INSERT INTO $table (canonical_key,connector_slug,domain_name,object_id,object_version,reason_class,received_at,purged_at,expires_at) VALUES (%s,%s,%s,%s,%d,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE object_version=GREATEST(object_version,VALUES(object_version)),reason_class=IF(VALUES(object_version) >= object_version,VALUES(reason_class),reason_class),received_at=IF(VALUES(object_version) >= object_version,VALUES(received_at),received_at),purged_at=IF(VALUES(object_version) >= object_version,VALUES(purged_at),purged_at),expires_at=IF(VALUES(object_version) >= object_version,VALUES(expires_at),expires_at)",
					$key, $connector, $domain, $object_id, $version, sanitize_key( $reason ), $now, $now, $expires
				);
				...
				$wpdb->query( 'ROLLBACK' );
				return new \WP_Error( 'file26_tombstone_failed', 'Document revocation could not be completed atomically.' );
			}
			if ( function_exists( 'wp_cache_flush_group' ) ) { wp_cache_flush_group( 'sabri_file26' ); } else { wp_cache_flush(); }
			$this->security->audit( 'search_document_tombstoned', array( 'object_type' => 'document', 'object_key' => $key, 'reason' => $reason, 'metadata' => array( 'version' => $version ) ) );
			do_action( 'sabri_file26_event', 'SearchDocumentTombstoned', array( 'contract_version' => SABRI_FILE26_CONTRACT_VERSION, 'canonical_key' => $key, 'object_version' => $version, 'reason_class' => sanitize_key( $reason ) ) );
			return true;
		} finally {
			$this->release_object_lock( $lock );
		}
	...
	private function upsert_node( array $document ) {
		...
		$sql = $wpdb->prepare(
			"INSERT INTO $table (node_key,node_type,canonical_url,visibility,state,locale,version,title,payload,updated_at) VALUES (%s,%s,%s,%s,%s,%s,%d,%s,%s,%s) ON DUPLICATE KEY UPDATE node_type=VALUES(node_type),canonical_url=VALUES(canonical_url),visibility=VALUES(visibility),state=VALUES(state),locale=VALUES(locale),version=VALUES(version),title=VALUES(title),payload=VALUES(payload),updated_at=VALUES(updated_at)",
			$document['canonical_key'], $document['entity_type'], $document['canonical_url'], $document['visibility'], $document['state'], $document['locale'], $document['object_version'], $document['title'], $document['payload'], $document['updated_at']
		);
	...
	public function enqueue_reindex( $connector_slug, array $scope = array() ) {
		...
		if ( ! $this->connectors->get( $connector_slug ) ) { return new \WP_Error( 'file26_unknown_connector', 'Unknown connector.' ); }
		$uuid = DB::uuid();
		$inserted = $wpdb->insert( DB::table( 'jobs' ), array( 'job_uuid' => $uuid, 'job_type' => 'shadow_reindex', 'status' => 'pending', 'scope_json' => wp_json_encode( array_merge( array( 'connector' => $connector_slug ), $scope ) ), 'cursor_value' => '', 'counts_json' => wp_json_encode( array( 'processed' => 0, 'failed' => 0 ) ), 'attempts' => 0, 'available_at' => DB::now(), 'created_at' => DB::now(), 'updated_at' => DB::now() ) );
		if ( false === $inserted ) { return new \WP_Error( 'file26_job_enqueue_failed', 'Reindex job could not be queued.' ); }
		return $uuid;
	}

	/** Legacy worker retained for compatibility; runtime cron is replaced by Queue_Truth. */
	public function process_queue() {
		...
		$job = $wpdb->get_row( "SELECT * FROM $table WHERE status IN ('pending','retry') AND available_at <= UTC_TIMESTAMP() ORDER BY id ASC LIMIT 1", ARRAY_A );
		if ( ! $job ) { return; }
		...
		if ( ! $connector || empty( $connector['list_batch'] ) || ! is_callable( $connector['list_batch'] ) ) { $this->fail_job( $job['id'], $token, 'connector_unavailable', (int) $job['attempts'] + 1 ); return; }
		try {
			$batch = call_user_func( $connector['list_batch'], $job['cursor_value'], 100, $scope );
			if ( ! is_array( $batch ) || ! isset( $batch['items'] ) || ! is_array( $batch['items'] ) ) { throw new \RuntimeException( 'Invalid connector batch.' ); }
			$counts = json_decode( $job['counts_json'], true );
			$counts = is_array( $counts ) ? $counts : array( 'processed' => 0, 'failed' => 0 );
			foreach ( $batch['items'] as $document ) { $result = $this->upsert( $document ); if ( is_wp_error( $result ) ) { $counts['failed']++; } else { $counts['processed']++; } }
			$done = ! empty( $batch['done'] );
			$saved = $wpdb->update( $table, array( 'status' => $done ? 'completed' : 'pending', 'cursor_value' => isset( $batch['next_cursor'] ) ? sanitize_text_field( $batch['next_cursor'] ) : '', 'counts_json' => wp_json_encode( $counts ), 'lock_token' => null, 'available_at' => DB::now(), 'finished_at' => $done ? DB::now() : null, 'updated_at' => DB::now() ), array( 'id' => $job['id'], 'lock_token' => $token ), array( '%s', '%s', '%s', '%s', '%s', '%s', '%s' ), array( '%d', '%s' ) );
			if ( 1 !== (int) $saved ) { $this->security->audit( 'search_job_lock_lost', array( 'object_type' => 'job', 'object_key' => $job['job_uuid'], 'reason' => 'completion_cas_failed' ) ); }
		} catch ( \Throwable $e ) {
			$this->fail_job( $job['id'], $token, 'job_exception', (int) $job['attempts'] + 1 );
		}
	}
	...
	private function fail_job( $id, $token, $code, $attempts ) {
		...
		$wpdb->update( DB::table( 'jobs' ), array( 'status' => $attempts >= 8 ? 'dead_letter' : 'retry', 'error_code' => sanitize_key( $code ), 'lock_token' => null, 'available_at' => gmdate( 'Y-m-d H:i:s', time() + $retry ), 'updated_at' => DB::now() ), array( 'id' => (int) $id, 'lock_token' => $token ) );
	}
	...
	private function release_object_lock( $lock_name ) {
		global $wpdb;
		if ( is_string( $lock_name ) && '' !== $lock_name ) { $wpdb->get_var( $wpdb->prepare( 'SELECT RELEASE_LOCK(%s)', $lock_name ) ); }
	}

	public function reconcile() {
		global $wpdb;
		$documents = DB::table( 'documents' );
		$tombstones = DB::table( 'tombstones' );
		...
		try {
			$valid_tombstone = "NOT EXISTS (SELECT 1 FROM $documents live WHERE live.canonical_key=t.canonical_key AND live.object_version>t.object_version)";
			$queries = array(
				"DELETE e FROM $edges e WHERE EXISTS (SELECT 1 FROM $tombstones t WHERE (e.source_key=t.canonical_key OR e.target_key=t.canonical_key) AND $valid_tombstone)",
				"DELETE c FROM $classes c INNER JOIN $tombstones t ON c.object_key=t.canonical_key WHERE $valid_tombstone",
				"DELETE n FROM $nodes n INNER JOIN $tombstones t ON n.node_key=t.canonical_key WHERE $valid_tombstone",
				...
				"DELETE e FROM $edges e LEFT JOIN $nodes s ON e.source_key=s.node_key LEFT JOIN $nodes target ON e.target_key=target.node_key WHERE s.node_key IS NULL OR target.node_key IS NULL",
			);
			...
			$wpdb->query( 'ROLLBACK' );
			return new \WP_Error( 'file26_reconcile_failed', 'Deletion and graph reconciliation failed atomically.' );
		}
	}
	...
	public function retention() {
		...
		$deleted = array();
		foreach ( $queries as $class => $sql ) {
			$result = $wpdb->query( $sql );
			if ( false === $result ) { $this->security->audit( 'search_retention_failed', array( 'object_type' => 'retention', 'object_key' => $class, 'reason' => 'db_delete_failed' ) ); return new \WP_Error( 'file26_retention_failed', 'File 26 retention could not complete all required deletion classes.' ); }
