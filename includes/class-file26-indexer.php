...
final class Indexer {
	...
	public function upsert( array $document ) {
		...
		try {
			...
			if ( $existing ) {
				...
				if ( (int)$document['object_version'] === (int)$existing['object_version'] && (int)$document['source_event_sequence'] === (int)$existing['source_event_sequence'] ) {
					$this->security->audit( 'index_event_conflict', array( 'object_type'=>'document','object_key'=>$document['canonical_key'],'reason'=>'same_version_sequence_different_checksum' ) );
					return new \WP_Error( 'file26_index_event_conflict', 'The same source version and event sequence produced different content; fail closed.', array( 'status' => 409 ) );
				}
			}
			$sql = $wpdb->prepare(
			...
			try {
				$ok = $wpdb->query( $sql ); if ( false === $ok ) { throw new \RuntimeException( 'Document projection write failed.' ); }
				...
				if ( ! $this->upsert_node( $document ) ) { throw new \RuntimeException( 'Knowledge node projection write failed.' ); }
				if ( false === $wpdb->query( 'COMMIT' ) ) { throw new \RuntimeException( 'Projection commit failed.' ); }
			} catch ( \Throwable $e ) {
				$wpdb->query( 'ROLLBACK' );
				return new \WP_Error( 'file26_index_write_failed', 'Search document could not be indexed atomically.' );
			}
	...
	private function sanitize_document( array $document ) {
		...
		$freshness=null;
		if(isset($document['freshness_at'])&&is_scalar($document['freshness_at'])&&''!==trim((string)$document['freshness_at'])){$freshness_ts=strtotime((string)$document['freshness_at']);if(false===$freshness_ts){return new \WP_Error('file26_invalid_freshness','Owner freshness timestamp is invalid; freshness cannot be fabricated.');}$freshness=gmdate('Y-m-d H:i:s',$freshness_ts);}
		$data=array('canonical_key'=>self::canonical_key($connector,$domain,$object_id),'connector_slug'=>$connector,'domain_name'=>$domain,'object_id'=>$object_id,'object_version'=>$version,'entity_type'=>sanitize_key($document['entity_type']),'locale'=>substr(sanitize_text_field($document['locale']),0,20),'state'=>$state,'visibility'=>$visibility,'title'=>$title,'excerpt'=>$excerpt,'normalized_title'=>$this->normalizer->normalize($title),'normalized_body'=>$this->normalizer->normalize($body),'canonical_url'=>$url,'author_key'=>isset($document['author_key'])?sanitize_text_field($document['author_key']):'','topic_ids'=>wp_json_encode(array_values(array_unique(array_map('sanitize_text_field',isset($document['topic_ids'])?(array)$document['topic_ids']:array())))),'country'=>isset($document['country'])?sanitize_text_field($document['country']):'','location'=>isset($document['location'])?sanitize_text_field($document['location']):'','availability'=>isset($document['availability'])?sanitize_key($document['availability']):'','quality_score'=>isset($document['quality_score'])?min(1,max(0,(float)$document['quality_score'])):0,'authority_score'=>isset($document['authority_score'])?min(1,max(0,(float)$document['authority_score'])):0,'popularity_score'=>isset($document['popularity_score'])?max(0,(float)$document['popularity_score']):0,'freshness_at'=>$freshness,'safety_class'=>isset($document['safety_class'])?sanitize_key($document['safety_class']):'general','payload'=>wp_json_encode($payload),'source_event_id'=>isset($document['source_event_id'])?sanitize_text_field($document['source_event_id']):'','source_event_sequence'=>$sequence,'indexed_at'=>$now,'updated_at'=>$now);
		$checksum_data=$data;unset($checksum_data['indexed_at'],$checksum_data['updated_at']);$data['checksum']=hash('sha256',wp_json_encode($checksum_data));return $data;
	}
	...
	public function enqueue_reindex($connector_slug,array $scope=array()){
		global $wpdb;$connector_slug=sanitize_key($connector_slug);if(!$this->connectors->get($connector_slug)){return new \WP_Error('file26_unknown_connector','Unknown connector.');}$scope=array_merge($scope,array('connector'=>$connector_slug));$uuid=DB::uuid();$inserted=$wpdb->insert(DB::table('jobs'),array('job_uuid'=>$uuid,'job_type'=>'shadow_reindex','status'=>'pending','scope_json'=>wp_json_encode($scope),'cursor_value'=>'','counts_json'=>wp_json_encode(array('processed'=>0,'failed'=>0)),'attempts'=>0,'available_at'=>DB::now(),'created_at'=>DB::now(),'updated_at'=>DB::now()));if(false===$inserted){return new \WP_Error('file26_job_enqueue_failed','Reindex job could not be queued.');}return $uuid;
	}
