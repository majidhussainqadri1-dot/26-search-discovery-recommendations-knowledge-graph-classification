...
final class Indexer {
	...
	public function upsert( array $document ) {
		...
		try {
			$tombstone_raw = $wpdb->get_var( $wpdb->prepare( 'SELECT object_version FROM ' . DB::table( 'tombstones' ) . ' WHERE canonical_key=%s', $document['canonical_key'] ) );
			if ( null === $tombstone_raw && '' !== $wpdb->last_error ) { return new \WP_Error( 'file26_tombstone_version_read_failed', 'Tombstone precedence could not be verified; indexing failed closed.' ); }
			$tombstone_version = null === $tombstone_raw ? 0 : (int) $tombstone_raw;
			...
			if ( null === $existing && '' !== $wpdb->last_error ) { return new \WP_Error( 'file26_document_version_read_failed', 'Existing search-document state could not be verified; indexing failed closed.' ); }
			if ( $existing ) {
				$incoming_version = (int) $document['object_version'];
				$existing_version = (int) $existing['object_version'];
				$incoming_sequence = (int) $document['source_event_sequence'];
				$existing_sequence = (int) $existing['source_event_sequence'];
				if ( $incoming_version < $existing_version ) { $this->security->audit( 'index_event_ignored', array( 'object_type'=>'document','object_key'=>$document['canonical_key'],'reason'=>'stale_object_version' ) ); return true; }
				if ( $incoming_version === $existing_version ) {
					if ( $incoming_sequence < $existing_sequence ) { return true; }
					if ( $incoming_sequence === $existing_sequence ) {
						if ( hash_equals( (string) $existing['checksum'], $document['checksum'] ) ) { return true; }
						$this->security->audit( 'index_event_conflict', array( 'object_type'=>'document','object_key'=>$document['canonical_key'],'reason'=>'same_version_sequence_different_checksum','metadata'=>array('object_version'=>$incoming_version,'source_event_sequence'=>$incoming_sequence) ) );
						return new \WP_Error( 'file26_conflicting_source_event', 'The same object version and event sequence produced conflicting content; indexing failed closed.' );
					}
				}
			}
			if ( false === $wpdb->query( 'START TRANSACTION' ) ) { return new \WP_Error( 'file26_index_transaction_unavailable', 'Search indexing transaction could not be started safely.' ); }
			try {
				$sql = $wpdb->prepare(
				...
				if ( false === $wpdb->delete( DB::table( 'tombstones' ), array( 'canonical_key'=>$document['canonical_key'] ), array('%s') ) ) { throw new \RuntimeException( 'Tombstone cleanup failed.' ); }
				if ( ! $this->upsert_node( $document ) ) { throw new \RuntimeException( 'Graph node write failed.' ); }
				if ( false === $wpdb->query( 'COMMIT' ) ) { throw new \RuntimeException( 'Index transaction commit failed.' ); }
			} catch ( \Throwable $e ) {
				$wpdb->query( 'ROLLBACK' );
	...
	private function sanitize_document( array $document ) {
		...
		$version = $this->strict_integer( $document['object_version'], 1 ); if ( null === $version ) { return new \WP_Error( 'file26_invalid_object_version', 'object_version must be a positive integer.' ); }
		$sequence = $version; if ( array_key_exists( 'source_event_sequence', $document ) ) { $sequence = $this->strict_integer( $document['source_event_sequence'], 0 ); if ( null === $sequence ) { return new \WP_Error( 'file26_invalid_source_event_sequence', 'source_event_sequence must be a non-negative integer.' ); } }
		$freshness_provided = isset( $document['freshness_at'] ) && '' !== trim( (string) $document['freshness_at'] );
		$freshness = DB::now();
		if ( $freshness_provided ) { $stamp = strtotime( (string) $document['freshness_at'] ); if ( false === $stamp || $stamp > time() + 300 ) { return new \WP_Error( 'file26_invalid_freshness', 'freshness_at must be a valid non-future date/time.' ); } $freshness = gmdate( 'Y-m-d H:i:s', $stamp ); }
		$connector=sanitize_key($document['connector_slug']); $domain=sanitize_key($document['domain']); $object_id=substr(sanitize_text_field((string)$document['object_id']),0,191); $title=sanitize_text_field($document['title']); $excerpt=isset($document['excerpt'])?wp_strip_all_tags($document['excerpt'],true):''; $body=isset($document['search_text'])?wp_strip_all_tags($document['search_text'],true):$excerpt;
		if ( '' === $connector || '' === $domain || '' === $object_id ) { return new \WP_Error( 'file26_invalid_canonical_identity', 'Connector, domain and object identity must remain non-empty after normalization.' ); }
		$payload=isset($document['payload'])&&is_array($document['payload'])?$this->sanitize_payload($document['payload']):array(); if ( is_wp_error( $payload ) ) { return $payload; }
		$now=DB::now();
		$data=array('canonical_key'=>self::canonical_key($connector,$domain,$object_id),'connector_slug'=>$connector,'domain_name'=>$domain,'object_id'=>$object_id,'object_version'=>$version,'entity_type'=>sanitize_key($document['entity_type']),'locale'=>substr(sanitize_text_field($document['locale']),0,20),'state'=>$state,'visibility'=>$visibility,'title'=>$title,'excerpt'=>$excerpt,'normalized_title'=>$this->normalizer->normalize($title),'normalized_body'=>$this->normalizer->normalize($body),'canonical_url'=>$url,'author_key'=>isset($document['author_key'])?substr(sanitize_text_field($document['author_key']),0,191):'','topic_ids'=>wp_json_encode(array_slice(array_values(array_unique(array_map('sanitize_text_field',isset($document['topic_ids'])?(array)$document['topic_ids']:array()))),0,100)),'country'=>isset($document['country'])?substr(sanitize_text_field($document['country']),0,191):'','location'=>isset($document['location'])?substr(sanitize_text_field($document['location']),0,191):'','availability'=>isset($document['availability'])?sanitize_key($document['availability']):'','quality_score'=>isset($document['quality_score'])?min(1,max(0,(float)$document['quality_score'])):0,'authority_score'=>isset($document['authority_score'])?min(1,max(0,(float)$document['authority_score'])):0,'popularity_score'=>isset($document['popularity_score'])?max(0,(float)$document['popularity_score']):0,'freshness_at'=>$freshness,'safety_class'=>isset($document['safety_class'])?sanitize_key($document['safety_class']):'general','payload'=>wp_json_encode($payload),'source_event_id'=>isset($document['source_event_id'])?substr(sanitize_text_field($document['source_event_id']),0,191):'','source_event_sequence'=>$sequence,'indexed_at'=>$now,'updated_at'=>$now);
		$checksum_data = $data; unset( $checksum_data['indexed_at'], $checksum_data['updated_at'] ); if ( ! $freshness_provided ) { unset( $checksum_data['freshness_at'] ); } $data['checksum']=hash('sha256',wp_json_encode($checksum_data)); return $data;
	}

	private function strict_integer( $value, $minimum = 0 ) {
		if ( is_int( $value ) ) { $number = $value; }
		elseif ( is_string( $value ) && preg_match( '/^[0-9]+$/', trim( $value ) ) ) { $number = (int) trim( $value ); }
		else { return null; }
		return $number >= (int) $minimum ? $number : null;
	}
	...
	private function sanitize_payload( array $payload ) {
		...
		foreach($allowed as $key){if(!array_key_exists($key,$payload)){continue;} if(in_array($key,array('image_url','download_url'),true)){$clean[$key]=$this->security->safe_resource_url($payload[$key],$key);}elseif(in_array($key,array('verified_author','verified_doctor','download_allowed','ai_generated'),true)){$boolean=$this->strict_bool($payload[$key]);if(null===$boolean){return new \WP_Error('file26_invalid_payload_boolean','Trust-bearing payload boolean is malformed.');}$clean[$key]=$boolean;}elseif('global_doctor_rank'===$key||'source_count'===$key){$clean[$key]=max(0,(int)$payload[$key]);}elseif('doctor_rank_score'===$key){$clean[$key]=min(100,max(0,(float)$payload[$key]));}elseif(preg_match('/_score$/',$key)){$value=(float)$payload[$key];$clean[$key]=is_finite($value)?min(1,max(0,$value)):0;}else{$clean[$key]=substr(sanitize_text_field($payload[$key]),0,512);}}
	...
	public function tombstone($connector,$domain,$object_id,$object_version,$reason='deleted'){
		global $wpdb; $version=$this->strict_integer($object_version,1);if(null===$version){return new \WP_Error('file26_invalid_object_version','object_version must be a positive integer.');}$connector=sanitize_key($connector);$domain=sanitize_key($domain);$object_id=substr(sanitize_text_field((string)$object_id),0,191);if(''===$connector||''===$domain||''===$object_id){return new \WP_Error('file26_invalid_canonical_identity','Connector, domain and object identity must remain non-empty after normalization.');}$key=self::canonical_key($connector,$domain,$object_id); $lock=$this->acquire_object_lock($key); if(is_wp_error($lock)){return $lock;}
		try{$table=DB::table('tombstones');$document_table=DB::table('documents');$existing_raw=$wpdb->get_var($wpdb->prepare("SELECT object_version FROM $document_table WHERE canonical_key=%s",$key));if(null===$existing_raw&&''!==$wpdb->last_error){return new \WP_Error('file26_tombstone_document_read_failed','Existing document version could not be verified; revocation failed closed.');}$existing_version=null===$existing_raw?0:(int)$existing_raw;if($existing_version>$version){return true;}$now=DB::now();$days=max(90,min(180,(int)DB::setting('tombstone_retention_days',180)));$expires=gmdate('Y-m-d H:i:s',time()+($days*DAY_IN_SECONDS));if(false===$wpdb->query('START TRANSACTION')){return new \WP_Error('file26_tombstone_transaction_unavailable','Document revocation transaction could not be started safely.');}try{$sql=$wpdb->prepare("INSERT INTO $table (canonical_key,connector_slug,domain_name,object_id,object_version,reason_class,received_at,purged_at,expires_at) VALUES (%s,%s,%s,%s,%d,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE object_version=GREATEST(object_version,VALUES(object_version)),reason_class=IF(VALUES(object_version) >= object_version,VALUES(reason_class),reason_class),received_at=IF(VALUES(object_version) >= object_version,VALUES(received_at),received_at),purged_at=IF(VALUES(object_version) >= object_version,VALUES(purged_at),purged_at),expires_at=IF(VALUES(object_version) >= object_version,VALUES(expires_at),expires_at)",$key,$connector,$domain,$object_id,$version,sanitize_key($reason),$now,$now,$expires);if(false===$wpdb->query($sql)){throw new \RuntimeException('Tombstone write failed.');}foreach(array(array($document_table,array('canonical_key'=>$key)),array(DB::table('nodes'),array('node_key'=>$key)),array(DB::table('classifications'),array('object_key'=>$key))) as $delete){if(false===$wpdb->delete($delete[0],$delete[1])){throw new \RuntimeException('Revocation delete failed.');}}if(false===$wpdb->query($wpdb->prepare('DELETE FROM '.DB::table('edges').' WHERE source_key=%s OR target_key=%s',$key,$key))){throw new \RuntimeException('Edge purge failed.');}if(false===$wpdb->query('COMMIT')){throw new \RuntimeException('Revocation commit failed.');}}catch(\Throwable $e){$wpdb->query('ROLLBACK');return new \WP_Error('file26_tombstone_failed','Document revocation could not be completed atomically.');}if(function_exists('wp_cache_flush_group')){wp_cache_flush_group('sabri_file26');}else{wp_cache_flush();}$this->security->audit('search_document_tombstoned',array('object_type'=>'document','object_key'=>$key,'reason'=>$reason,'metadata'=>array('version'=>$version)));do_action('sabri_file26_event','SearchDocumentTombstoned',array('contract_version'=>SABRI_FILE26_CONTRACT_VERSION,'canonical_key'=>$key,'object_version'=>$version,'reason_class'=>sanitize_key($reason)));return true;}finally{$this->release_object_lock($lock);}
	}
	...
	public function process_queue(){
		...
		if(false===$wpdb->query($wpdb->prepare("UPDATE $table SET status=IF(attempts>=8,'dead_letter','retry'),error_code='worker_timeout',lock_token=NULL,available_at=%s,finished_at=IF(attempts>=8,%s,NULL),updated_at=%s WHERE status='running' AND started_at<%s",$now,$now,$now,$stale_before))){$this->security->audit('search_job_recovery_failed',array('object_type'=>'job','reason'=>'stale_worker_update_failed'));return;}
	...
	public function reconcile(){
		global $wpdb;$documents=DB::table('documents');$tombstones=DB::table('tombstones');$nodes=DB::table('nodes');$classes=DB::table('classifications');$edges=DB::table('edges');if(false===$wpdb->query('START TRANSACTION')){return new \WP_Error('file26_reconcile_transaction_unavailable','Deletion and graph reconciliation transaction could not be started safely.');}
		try{$queries=array("DELETE e FROM $edges e INNER JOIN $tombstones t ON (e.source_key=t.canonical_key OR e.target_key=t.canonical_key)","DELETE c FROM $classes c INNER JOIN $tombstones t ON c.object_key=t.canonical_key","DELETE n FROM $nodes n INNER JOIN $tombstones t ON n.node_key=t.canonical_key","DELETE d FROM $documents d INNER JOIN $tombstones t ON d.canonical_key=t.canonical_key WHERE t.object_version >= d.object_version","DELETE e FROM $edges e LEFT JOIN $nodes s ON e.source_key=s.node_key LEFT JOIN $nodes t ON e.target_key=t.node_key WHERE s.node_key IS NULL OR t.node_key IS NULL");foreach($queries as $sql){if(false===$wpdb->query($sql)){throw new \RuntimeException('Reconciliation query failed.');}}if(false===$wpdb->query('COMMIT')){throw new \RuntimeException('Reconciliation commit failed.');}return array('reconciled'=>true,'timestamp_utc'=>DB::now());}catch(\Throwable $e){$wpdb->query('ROLLBACK');return new \WP_Error('file26_reconcile_failed','Deletion and graph reconciliation failed atomically.');}
	}

	public function retention(){global $wpdb;$failures=0;foreach(array('tombstones','feedback','rate_limits') as $name){$column='tombstones'===$name?'expires_at':'expires_at';if(false===$wpdb->query('DELETE FROM '.DB::table($name)." WHERE $column < UTC_TIMESTAMP()")){$failures++;}}$audit_days=max(365,(int)DB::setting('audit_retention_days',760));if(false===$wpdb->query($wpdb->prepare('DELETE FROM '.DB::table('audit').' WHERE created_at < %s',gmdate('Y-m-d H:i:s',time()-($audit_days*DAY_IN_SECONDS))))){$failures++;}if($failures){$this->security->audit('retention_partial_failure',array('object_type'=>'retention','metadata'=>array('failed_queries'=>$failures)));}}
}
