...
final class Indexer {
	...
	private function sanitize_document( array $document ) {
		...
		if ( ! in_array( $state, $allowed_states, true ) || ! in_array( $visibility, $allowed_visibility, true ) ) {
			return new \WP_Error( 'file26_invalid_visibility_state', 'Unknown state or visibility; fail closed.' );
		}
		$url = $this->security->safe_url( $document['canonical_url'] );
		if ( ! $url ) {
			return new \WP_Error( 'file26_invalid_canonical_url', 'Canonical URL must be a safe same-origin route.' );
		}
		...
		$now = DB::now();
		$data = array(
			'canonical_key' => self::canonical_key( $connector, $domain, $object_id ),
			'connector_slug' => $connector,
			'domain_name' => $domain,
			'object_id' => $object_id,
			'object_version' => $version,
			'entity_type' => sanitize_key( $document['entity_type'] ),
			'locale' => substr( sanitize_text_field( $document['locale'] ), 0, 20 ),
			'state' => $state,
			'visibility' => $visibility,
			'title' => $title,
			'excerpt' => $excerpt,
			'normalized_title' => $this->normalizer->normalize( $title ),
			'normalized_body' => $this->normalizer->normalize( $body ),
			'canonical_url' => $url,
			'author_key' => isset( $document['author_key'] ) ? sanitize_text_field( $document['author_key'] ) : '',
			...
			'country' => isset( $document['country'] ) ? sanitize_text_field( $document['country'] ) : '',
			'location' => isset( $document['location'] ) ? sanitize_text_field( $document['location'] ) : '',
			'availability' => isset( $document['availability'] ) ? sanitize_key( $document['availability'] ) : '',
			'quality_score' => isset( $document['quality_score'] ) ? min( 1, max( 0, (float) $document['quality_score'] ) ) : 0,
			'authority_score' => isset( $document['authority_score'] ) ? min( 1, max( 0, (float) $document['authority_score'] ) ) : 0,
			'popularity_score' => isset( $document['popularity_score'] ) ? max( 0, (float) $document['popularity_score'] ) : 0,
			'freshness_at' => isset( $document['freshness_at'] ) ? gmdate( 'Y-m-d H:i:s', strtotime( $document['freshness_at'] ) ) : $now,
			'safety_class' => isset( $document['safety_class'] ) ? sanitize_key( $document['safety_class'] ) : 'general',
			'payload' => wp_json_encode( $payload ),
			'source_event_id' => isset( $document['source_event_id'] ) ? sanitize_text_field( $document['source_event_id'] ) : '',
	...
	private function sanitize_payload( array $payload ) {
		$allowed = array(
			...
			'ai_generated',
			'ai_provider_label',
			'institutional_account_class',
			'qualification_score',
			'experience_score',
			'patient_verified_review_score',
			'ethical_conduct_score',
			'knowledge_contribution_score',
			'responsiveness_score',
			'profile_completeness_score',
			'complaint_appeal_outcome_score',
			'manipulation_resistant_engagement_score',
			'doctor_rank_score',
			'doctor_rank_policy_version',
		);
		$clean = array();
		foreach ( $allowed as $key ) {
			...
			if ( in_array( $key, array( 'image_url', 'download_url' ), true ) ) {
				$clean[ $key ] = $this->security->safe_resource_url( $payload[ $key ], $key );
			} elseif ( in_array( $key, array( 'verified_author', 'verified_doctor', 'download_allowed', 'ai_generated' ), true ) ) {
				$clean[ $key ] = (bool) $payload[ $key ];
			} elseif ( 'global_doctor_rank' === $key || 'source_count' === $key ) {
				$clean[ $key ] = max( 0, (int) $payload[ $key ] );
			} elseif ( 'doctor_rank_score' === $key ) {
				$clean[ $key ] = min( 100, max( 0, (float) $payload[ $key ] ) );
			} elseif ( preg_match( '/_score$/', $key ) ) {
				$clean[ $key ] = min( 1, max( 0, (float) $payload[ $key ] ) );
			} else {
				$clean[ $key ] = sanitize_text_field( $payload[ $key ] );
			}
		}
		if ( empty( $clean['download_allowed'] ) ) {
			unset( $clean['download_url'] );
		}
		return $clean;
	}
	...
	private function upsert_node( array $document ) {
		...
		$sql = $wpdb->prepare(
			"INSERT INTO $table
			(node_key,node_type,canonical_url,visibility,state,locale,version,title,payload,updated_at)
	...
	public function enqueue_reindex( $connector_slug, array $scope = array() ) {
		...
		if ( ! $this->connectors->get( $connector_slug ) ) {
			return new \WP_Error( 'file26_unknown_connector', 'Unknown connector.' );
		}
		$uuid = DB::uuid();
		$wpdb->insert(
			DB::table( 'jobs' ),
			array(
				'job_uuid' => $uuid,
				'job_type' => 'shadow_reindex',
				'status' => 'pending',
				'scope_json' => wp_json_encode( array_merge( array( 'connector' => $connector_slug ), $scope ) ),
				'cursor_value' => '',
				'counts_json' => wp_json_encode( array( 'processed' => 0, 'failed' => 0 ) ),
				'attempts' => 0,
				'available_at' => DB::now(),
				'created_at' => DB::now(),
				'updated_at' => DB::now(),
	...
	public function process_queue() {
		...
		$table = DB::table( 'jobs' );
		$job = $wpdb->get_row(
			"SELECT * FROM $table WHERE status IN ('pending','retry') AND available_at <= UTC_TIMESTAMP() ORDER BY id ASC LIMIT 1", // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			ARRAY_A
		);
		if ( ! $job ) {
			return;
		...
		$locked = $wpdb->query(
			$wpdb->prepare(
				"UPDATE $table SET status='running',lock_token=%s,started_at=%s,attempts=attempts+1,updated_at=%s
		...
		if ( ! $connector || empty( $connector['list_batch'] ) || ! is_callable( $connector['list_batch'] ) ) {
			$this->fail_job( $job['id'], $token, 'connector_unavailable', (int) $job['attempts'] + 1 );
			return;
		}
		try {
	...
	public function reconcile() {
		...
		$wpdb->query(
			"DELETE d FROM $documents d INNER JOIN $tombstones t ON d.canonical_key=t.canonical_key WHERE t.object_version >= d.object_version" // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		);
		$wpdb->query(
			'DELETE e FROM ' . DB::table( 'edges' ) . ' e
			LEFT JOIN ' . DB::table( 'nodes' ) . ' s ON e.source_key=s.node_key
			LEFT JOIN ' . DB::table( 'nodes' ) . ' t ON e.target_key=t.node_key
	...
	public function retention() {
		global $wpdb;
		$wpdb->query( 'DELETE FROM ' . DB::table( 'tombstones' ) . ' WHERE expires_at < UTC_TIMESTAMP()' );
		$wpdb->query( 'DELETE FROM ' . DB::table( 'feedback' ) . ' WHERE expires_at < UTC_TIMESTAMP()' );
		$wpdb->query( 'DELETE FROM ' . DB::table( 'rate_limits' ) . ' WHERE expires_at < UTC_TIMESTAMP()' );
		$audit_days = max( 365, (int) DB::setting( 'audit_retention_days', 760 ) );
		$wpdb->query(
			$wpdb->prepare(
				'DELETE FROM ' . DB::table( 'audit' ) . ' WHERE created_at < %s',
