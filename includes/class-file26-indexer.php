...
final class Indexer {
	...
	private function sanitize_document( array $document ) {
		...
		$connector=sanitize_key($document['connector_slug']); $domain=sanitize_key($document['domain']); $object_id=sanitize_text_field((string)$document['object_id']);
	...
	private function fail_job($id,$token,$code,$attempts){global $wpdb;$retry=min(DAY_IN_SECONDS,(int)pow(2,min(10,$attempts))*60);$wpdb->update(DB::table('jobs'),array('status'=>$attempts>=8?'dead_letter':'retry','error_code'=>sanitize_key($code),'lock_token'=>null,'available_at'=>gmdate('Y-m-d H:i:s',time()+$retry),'updated_at'=>DB::now()),array('id'=>(int)$id,'lock_token'=>$token));}
	...
	public function reconcile(){
		...
		try{
			...
			if(false===$wpdb->query('COMMIT')){throw new \RuntimeException('Reconciliation commit failed.');}return array('reconciled'=>true,'timestamp_utc'=>DB::now());
