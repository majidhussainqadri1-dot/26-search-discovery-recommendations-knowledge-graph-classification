...
final class Indexer {
	...
	public function upsert( array $document ) {
		...
		try {
			...
			try {
				...
				$wpdb->query( 'ROLLBACK' );
				return new \WP_Error( 'file26_index_write_failed', 'Search document and its graph projection could not be indexed atomically.' );
			}
	...
	private function sanitize_payload( array $payload ) {
		// Capability-bearing signed delivery URLs are intentionally excluded; the canonical owner issues them after click-time authorization.
	...
	public function tombstone($connector,$domain,$object_id,$object_version,$reason='deleted'){
		...
		try{
			...
			$existing_version=(int)$wpdb->get_var($wpdb->prepare("SELECT object_version FROM $document_table WHERE canonical_key=%s",$key));
			...
			$table=DB::table('tombstones');$now=DB::now();$days=max(90,min(180,(int)DB::setting('tombstone_retention_days',180)));$expires=gmdate('Y-m-d H:i:s',time()+($days*DAY_IN_SECONDS));
			$wpdb->query('START TRANSACTION');
			try{
				...
				$wpdb->query('COMMIT');
			}catch(\Throwable $e){$wpdb->query('ROLLBACK');return new \WP_Error('file26_tombstone_failed','Document revocation and all derivatives could not be completed atomically.');}
	...
	public function reconcile(){
		...
		}catch(\Throwable $e){$wpdb->query('ROLLBACK');return new \WP_Error('file26_reconcile_failed','Deletion, graph and unsafe-payload reconciliation failed atomically.');}
	}
