...
final class Governance {
	...
	public function transition_connector( $slug, $target, $reason ) {
		...
		$this->security->audit( 'connector_' . $target, array(
			'object_type' => 'connector', 'object_key' => $slug, 'reason' => $reason,
			'metadata' => array( 'from' => $row['status'], 'to' => $target ),
		) );
	...
	public function stage_ranking_policy( array $input ) {
		...
		if ( ! $version || ! $features ) {
			return new \WP_Error( 'file26_invalid_policy', 'Version and bounded feature definitions are required.', array( 'status' => 400 ) );
		}
		...
		$this->security->audit( 'ranking_policy_staged', array(
			'object_type' => 'ranking_policy', 'object_key' => $uuid,
			'metadata' => array( 'context' => $context, 'audience' => $audience, 'version' => $version ),
		) );
		return $uuid;
	}
	...
	public function activate_ranking_policy( $uuid, $second_approver_id, $reason = '' ) {
		...
		try {
			$wpdb->update(
				...
				array( 'status' => 'rolled_back', 'updated_at' => DB::now() ),
				array( 'context_name' => $row['context_name'], 'audience' => $row['audience'], 'status' => 'active' )
			);
			$updated = $wpdb->update(
				DB::table( 'ranking_policies' ),
				array( 'status' => 'active', 'approval_two' => $second, 'effective_at' => DB::now(), 'updated_at' => DB::now() ),
				array( 'policy_uuid' => $uuid, 'status' => 'staged' ),
				array( '%s', '%d', '%s', '%s' ), array( '%s', '%s' )
			);
			if ( 1 !== $updated ) {
		...
		$this->security->audit( 'ranking_policy_activated', array(
			'object_type' => 'ranking_policy', 'object_key' => $uuid, 'reason' => sanitize_text_field( $reason ),
			'metadata' => array( 'second_approver' => $second, 'version' => $row['version'] ),
		) );
	...
	/**
	 * Restore the most recent previously active policy. The search service remains
	 * available; rollback never disables the whole module as a side effect.
	 */
	public function rollback_ranking_policy( $uuid, $reason, $second_approver_id = 0 ) {
		...
		$second = absint( $second_approver_id );
		if ( ! $second || $second === get_current_user_id() || ! apply_filters( 'sabri_file26_validate_ranking_approver', user_can( $second, 'approve_sabri_ranking' ), $second, $row ) ) {
			return new \WP_Error( 'file26_dual_approval_required', 'A distinct authorized second approver is required for rollback.', array( 'status' => 409 ) );
		}
		$previous = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM ' . DB::table( 'ranking_policies' ) . " WHERE context_name=%s AND audience=%s AND status='rolled_back' AND policy_uuid<>%s ORDER BY effective_at DESC,id DESC LIMIT 1",
				$row['context_name'], $row['audience'], $row['policy_uuid']
			),
			ARRAY_A
		);
		if ( ! $previous ) {
			return new \WP_Error( 'file26_no_previous_policy', 'No previously active policy is available to restore.', array( 'status' => 409 ) );
		}
		$wpdb->query( 'START TRANSACTION' );
		try {
			$current_updated = $wpdb->update(
				DB::table( 'ranking_policies' ),
				array( 'status' => 'rolled_back', 'updated_at' => DB::now() ),
				array( 'policy_uuid' => $row['policy_uuid'], 'status' => 'active' ),
				array( '%s', '%s' ), array( '%s', '%s' )
			);
			$previous_updated = $wpdb->update(
				DB::table( 'ranking_policies' ),
				array( 'status' => 'active', 'approval_two' => $second, 'effective_at' => DB::now(), 'updated_at' => DB::now() ),
				array( 'policy_uuid' => $previous['policy_uuid'], 'status' => 'rolled_back' ),
				array( '%s', '%d', '%s', '%s' ), array( '%s', '%s' )
			);
			if ( 1 !== $current_updated || 1 !== $previous_updated ) {
				throw new \RuntimeException( 'Concurrent rollback transition.' );
			}
			if ( 'search' === $row['context_name'] && 'public' === $row['audience'] ) {
		...
		$this->security->audit( 'ranking_policy_rolled_back', array(
			'object_type' => 'ranking_policy', 'object_key' => $row['policy_uuid'], 'reason' => sanitize_text_field( $reason ),
			'metadata' => array(
				'restored_policy_uuid' => $previous['policy_uuid'], 'restored_version' => $previous['version'],
				'second_approver' => $second,
			),
		) );
		do_action( 'sabri_file26_event', 'RankingPolicyRolledBack', array(
			'policy_uuid' => $row['policy_uuid'], 'restored_policy_uuid' => $previous['policy_uuid'],
	...
	public function review_classification( $object_key, $term_uuid, $decision, $reason = '' ) {
		...
		if ( ! in_array( $decision, array( 'approved', 'rejected', 'corrected', 'removed' ), true ) ) {
			return new \WP_Error( 'file26_invalid_classification_decision', 'Invalid classification decision.', array( 'status' => 400 ) );
		}
		...
		$term_exists = (bool) $wpdb->get_var( $wpdb->prepare( 'SELECT 1 FROM ' . DB::table( 'terms' ) . " WHERE term_uuid=%s AND status IN ('active','deprecated','merged','split') LIMIT 1", $term_uuid ) );
		if ( ! $term_exists ) {
			return new \WP_Error( 'file26_term_not_found', 'Classification term not found.', array( 'status' => 404 ) );
		}
		$updated = $wpdb->query( $wpdb->prepare(
			'UPDATE ' . DB::table( 'classifications' ) . ' SET status=%s,reviewer_id=%d,version=version+1,updated_at=%s WHERE object_key=%s AND term_uuid=%s',
			$decision, get_current_user_id(), DB::now(), $object_key, $term_uuid
		) );
		if ( ! $updated ) {
			return new \WP_Error( 'file26_classification_not_found', 'Classification assignment not found.', array( 'status' => 404 ) );
		}
		$this->security->audit( 'classification_' . $decision, array(
			'object_type' => 'classification', 'object_key' => $object_key . ':' . $term_uuid,
			'reason' => sanitize_text_field( $reason ),
		) );
	...
	public function transition_edge( $edge_uuid, $target, $reason = '' ) {
		...
		$updated = $wpdb->query( $wpdb->prepare(
			'UPDATE ' . DB::table( 'edges' ) . ' SET state=%s,version=version+1,updated_at=%s WHERE edge_uuid=%s AND state=%s AND version=%d',
			$target, DB::now(), $edge_uuid, $row['state'], (int) $row['version']
		) );
		if ( 1 !== $updated ) {
			return new \WP_Error( 'file26_edge_transition_conflict', 'Graph edge changed concurrently.', array( 'status' => 409 ) );
		}
		$this->security->audit( 'knowledge_edge_' . $target, array(
			'object_type' => 'knowledge_edge', 'object_key' => $edge_uuid, 'reason' => sanitize_text_field( $reason ),
			'metadata' => array( 'from' => $row['state'], 'to' => $target ),
		) );
	...
	private function sanitize_policy_features( array $features, $depth = 0 ) {
		...
		$forbidden = array(
			'donation', 'payment', 'purchase', 'paid_promotion', 'advertising_spend', 'founder_favoritism',
			'religion_inference', 'clinical_query', 'message_body', 'patient_record', 'identity_document',
		);
		$clean = array();
