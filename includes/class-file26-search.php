...
final class Search {
	...
	public function run( array $request ) {
		...
		while ( $scan_offset < $max_scan ) {
			...
			if ( ! is_array( $rows ) ) {
				$this->security->audit( 'search_candidate_read_failed', array( 'object_type' => 'search', 'object_key' => 'candidate-read', 'reason' => 'db_read_failed', 'trace_id' => $trace ) );
				return new \WP_Error( 'file26_search_read_failed', 'Search candidates could not be read safely. Please retry.', array( 'status' => 503, 'trace_id' => $trace ) );
			}
			if ( empty( $rows ) ) {
				break;
			}
		...
		if ( $public_cache ) { wp_cache_set( $cache_key, $response, 'sabri_file26', 300 ); }
		return $response;
	}
	...
	private function apply_graph_relationship_scores( array $rows ) {
		...
		$index = array();
		foreach ( $rows as $position => $row ) { $index[ $row['canonical_key'] ] = $position; }
		...
		foreach ( array_chunk( $keys, 200 ) as $chunk ) {
			...
			foreach ( (array) $edges as $edge ) {
				if ( isset( $seen[ $edge['edge_uuid'] ] ) ) { continue; }
	...
	private function sanitize_filters( $filters ) {
		...
		foreach ( array( 'entity_type', 'country', 'location', 'availability', 'connector', 'domain', 'topic', 'sort', 'language' ) as $key ) {
			if ( ! empty( $filters[ $key ] ) ) { $clean[ $key ] = 'language' === $key ? substr( sanitize_text_field( $filters[ $key ] ), 0, 20 ) : sanitize_key( $filters[ $key ] ); }
		}
		if ( ! empty( $filters['author'] ) ) { $clean['author'] = sanitize_text_field( $filters['author'] ); }
		foreach ( array( 'date_from', 'date_to' ) as $date_key ) {
			if ( ! empty( $filters[ $date_key ] ) && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $filters[ $date_key ] ) ) { $clean[ $date_key ] = $filters[ $date_key ]; }
		}
		return $clean;
	}
	...
	private function to_result( array $row, $query ) {
		...
		if ( ! empty( $payload['download_allowed'] ) && ! empty( $payload['download_url'] ) ) {
			$actions['download'] = array( 'url' => $payload['download_url'], 'label' => ! empty( $payload['download_label'] ) ? $payload['download_label'] : __( 'Download', 'sabri-file26' ), 'icon' => 'download' );
		}
		return array(
			'key' => $row['canonical_key'], 'entity_type' => $row['entity_type'], 'title' => $row['title'], 'excerpt' => $row['excerpt'], 'url' => $row['canonical_url'], 'locale' => $row['locale'],
			'state' => $row['state'], 'author_key' => $row['author_key'], 'topics' => $row['topic_ids_array'], 'country' => $row['country'], 'location' => $row['location'], 'availability' => $row['availability'],
			'score' => isset( $row['_score'] ) ? (float) $row['_score'] : 0.0, 'explanation_codes' => $this->explanation_codes( $row, $query ),
			'doctor_tier' => 'doctor' === $row['entity_type'] ? $this->ranking->doctor_tier( $payload ) : null, 'payload' => $payload, 'actions' => $actions,
		);
	}
	...
	private function explanation_codes( array $row, $query ) {
		...
		foreach ( $this->normalizer->phrases( $query ) as $phrase ) {
			if ( false !== $this->strpos( $row['normalized_title'], $phrase ) || false !== $this->strpos( $row['normalized_body'], $phrase ) ) { $codes[] = 'exact_phrase'; break; }
		}
		if ( (float) $row['authority_score'] >= 0.7 ) { $codes[] = 'verified_authority'; }
		if ( (float) $row['quality_score'] >= 0.7 ) { $codes[] = 'source_quality'; }
		if ( ! empty( $row['payload']['relationship_score'] ) ) { $codes[] = 'knowledge_graph_relation'; }
		if ( in_array( $row['state'], array( 'corrected', 'retracted' ), true ) ) { $codes[] = 'status_disclosed'; }
	...
	private function facets_from_rows( array $rows ) {
		...
		foreach ( $rows as $row ) {
			foreach ( array( 'entity_type', 'country', 'locale', 'availability' ) as $field ) {
				...
				if ( '' !== $value ) { $facets[ $field ][ $value ] = isset( $facets[ $field ][ $value ] ) ? $facets[ $field ][ $value ] + 1 : 1; }
			}
			foreach ( isset( $row['topic_ids_array'] ) ? $row['topic_ids_array'] : array() as $topic ) {
				$topic = sanitize_text_field( $topic ); if ( $topic ) { $facets['topic'][ $topic ] = isset( $facets['topic'][ $topic ] ) ? $facets['topic'][ $topic ] + 1 : 1; }
			}
		}
	...
	private function record_metric( $query, $locale, $count, $trace, $latency_ms ) {
		...
		$sql = $wpdb->prepare(
			"INSERT INTO $table (metric_date,metric_key,bucket_hash,locale,count_value,sum_value,updated_at) VALUES (%s,%s,%s,%s,1,%f,%s) ON DUPLICATE KEY UPDATE count_value=count_value+1,sum_value=sum_value+VALUES(sum_value),updated_at=VALUES(updated_at)",
			gmdate( 'Y-m-d' ), $count ? 'search_with_results' : 'search_zero_result', $bucket, substr( sanitize_text_field( $locale ), 0, 20 ), max( 0, (float) $latency_ms ), DB::now()
		);
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}

	private function strpos( $haystack, $needle ) {
		if ( function_exists( 'mb_strpos' ) ) { return mb_strpos( (string) $haystack, (string) $needle, 0, 'UTF-8' ); }
