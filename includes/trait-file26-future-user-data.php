...
/** F26-FUT-16 — saved search alert registry delegates delivery to File 19. */
	public function saved_search_alerts( \WP_REST_Request $request ) {
		...
		$id = $id ?: sanitize_key( wp_generate_uuid4() ); $alerts[ $id ] = array( 'alert_id' => $id, 'query' => $q, 'query_hash' => hash( 'sha256', $q ), 'filters' => $alert_filters, 'cadence' => $cadence, 'enabled' => ! isset( $params['enabled'] ) || (bool) $params['enabled'], 'updated_at' => gmdate( 'c' ) );
		$saved = $this->save_user_meta_cas( $user_id, self::META_ALERTS, $stored, $alerts ); if ( is_wp_error( $saved ) ) { return $saved; } do_action( 'sabri_file26_saved_search_alert_changed', $user_id, $id, $alerts[ $id ] ); return array( 'saved' => true, 'alert' => $alerts[ $id ], 'delivery_owner' => 'File 19', 'file26_sends_notifications' => false );
	}
