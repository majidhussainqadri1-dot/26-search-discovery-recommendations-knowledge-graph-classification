...
trait Future_User_Data_Trait {
	...
	public function saved_search_alerts( \WP_REST_Request $request ) {
		...
		$cadence = isset( $params['cadence'] ) ? sanitize_key( (string) $params['cadence'] ) : 'daily'; if ( ! in_array( $cadence, array( 'hourly', 'daily', 'weekly' ), true ) ) { return new \WP_Error( 'file26_alert_cadence_invalid', 'Saved-alert cadence must be hourly, daily, or weekly.', array( 'status' => 400 ) ); }
