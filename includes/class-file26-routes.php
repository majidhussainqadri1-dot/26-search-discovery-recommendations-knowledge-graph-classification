...
final class Routes {
	...
	public function __construct( Search $search, Recommendations $recommendations, Taxonomy $taxonomy ) {
		$this->search = $search; $this->recommendations = $recommendations; $this->taxonomy = $taxonomy;
	}
	...
	public function query_vars( $vars ) { $vars[] = 'sabri_f26_route'; $vars[] = 'sabri_f26_term'; return $vars; }
	...
	public function enqueue_assets() {
		...
		wp_localize_script( 'sabri-file26', 'SabriFile26', array( 'restUrl' => esc_url_raw( rest_url( 'sabri-search/v1/' ) ), 'nonce' => wp_create_nonce( 'wp_rest' ), 'loggedIn' => is_user_logged_in(), 'strings' => array( 'working' => __( 'Working…', 'sabri-file26' ), 'done' => __( 'Saved', 'sabri-file26' ), 'error' => __( 'The action could not be completed.', 'sabri-file26' ), 'undo' => __( 'Undo', 'sabri-file26' ) ) ) );
	}
	...
	public function search_shortcode( $atts = array() ) {
		...
		$filters = array(
			'entity_type' => $this->get_query_value( 'type', 'key' ), 'country' => $this->get_query_value( 'country', 'text' ), 'location' => $this->get_query_value( 'location', 'text' ), 'availability' => $this->get_query_value( 'availability', 'key' ), 'connector' => $this->get_query_value( 'connector', 'key' ), 'domain' => $this->get_query_value( 'domain', 'key' ), 'topic' => $this->get_query_value( 'topic', 'text' ), 'sort' => $this->get_query_value( 'sort', 'key' ), 'author' => $this->get_query_value( 'author', 'text' ), 'language' => $this->get_query_value( 'language', 'text' ), 'date_from' => $this->get_query_value( 'date_from', 'date' ), 'date_to' => $this->get_query_value( 'date_to', 'date' ),
		);
		$filters = array_filter( $filters, static function ( $value ) { return '' !== $value; } );
		$request = array( 'q' => $q, 'locale' => determine_locale(), 'limit' => 20, 'cursor' => isset( $_GET['cursor'] ) ? sanitize_text_field( wp_unslash( $_GET['cursor'] ) ) : '', 'filters' => $filters ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$data = $this->search->run( $request ); return $this->render( 'search', array( 'data' => $data, 'query' => $q, 'filters' => $filters ) );
	}

	public function discover_shortcode( $atts = array() ) { $this->enqueue_assets(); $data = $this->recommendations->get( array( 'context' => 'discover', 'limit' => 12 ) ); return $this->render( 'discover', array( 'data' => $data ) ); }

	public function topic_shortcode( $atts = array() ) {
		$this->enqueue_assets(); $atts = shortcode_atts( array( 'term' => '' ), $atts, 'sabri_topic' ); $term = $this->taxonomy->get( $atts['term'] );
		if ( ! $term || ! in_array( $term['status'], array( 'active', 'merged' ), true ) ) { return '<div class="sabri-f26-state" role="status">' . esc_html__( 'This topic is unavailable.', 'sabri-file26' ) . '</div>'; }
		if ( 'merged' === $term['status'] && $term['redirect_uuid'] ) {
			$target = $this->taxonomy->get( $term['redirect_uuid'] );
			if ( ! $target || 'active' !== $target['status'] ) { return '<div class="sabri-f26-state" role="status">' . esc_html__( 'This topic is unavailable.', 'sabri-file26' ) . '</div>'; }
			if ( get_query_var( 'sabri_f26_route' ) ) { wp_safe_redirect( home_url( '/topics/' . $target['slug'] . '/' ), 301 ); exit; }
			$term = $target;
		}
		$data = $this->search->run( array( 'q' => '', 'locale' => $term['language'], 'limit' => 20, 'filters' => array( 'topic' => $term['term_uuid'] ) ) );
	...
	private function get_query_value( $name, $type ) {
		...
		if ( 'date' === $type ) { $value = sanitize_text_field( $value ); return preg_match( '/^\d{4}-\d{2}-\d{2}$/', $value ) ? $value : ''; }
