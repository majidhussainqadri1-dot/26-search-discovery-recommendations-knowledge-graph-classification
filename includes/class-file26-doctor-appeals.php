...
final class Doctor_Appeals {
	...
	public function submit( $doctor_key, $reason, array $evidence = array() ) {
		...
		try {
			...
			$this->security->audit( 'doctor_ranking_appeal_submitted', array( 'object_type' => 'ranking_appeal', 'object_key' => $uuid, 'metadata' => array( 'doctor_key' => $doctor_key, 'policy_version' => $policy ) ) ); do_action( 'sabri_file26_event', 'DoctorRankingAppealSubmitted', array( 'appeal_uuid' => $uuid, 'doctor_key' => $doctor_key ) ); return array( 'appeal_uuid' => $uuid, 'status' => 'submitted', 'version' => 1 );
	...
	public function review( $appeal_uuid, $decision, $reason, $expected_version ) {
		...
		$this->security->audit( 'doctor_ranking_appeal_' . $decision, array( 'object_type' => 'ranking_appeal', 'object_key' => $appeal_uuid, 'reason' => $reason, 'metadata' => array( 'doctor_key' => $row['doctor_key'], 'version' => $version + 1 ) ) ); if ( 'corrected' === $decision ) { do_action( 'sabri_file26_doctor_ranking_recompute_requested', $row['doctor_key'], $appeal_uuid ); } do_action( 'sabri_file26_event', 'DoctorRankingAppealReviewed', array( 'appeal_uuid' => $appeal_uuid, 'decision' => $decision ) );
	...
	public function own() {
		...
		return null === $rows ? new \WP_Error( 'file26_appeal_read_failed', 'Ranking appeals could not be read.', array( 'status' => 500 ) ) : $rows;
	}
}
